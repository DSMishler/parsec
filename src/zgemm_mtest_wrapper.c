...
/* no GPU stuff for now */
#if 0 /* defined(DPLASMA_HAVE_CUDA) */
static parsec_taskpool_t*
                       ...
                       dplasma_info_t opt)
{
    ...
    {
        parsec_zgemm_NN_mtest_gpu_taskpool_t *tp;
        tp = parsec_zgemm_NN_mtest_gpu_new(transA, transB, alpha, beta,
...
#endif /* DPLASMA_HAVE_CUDA */
        zgemm_tp = dplasma_Zgemm_New_mtest(transA, transB, alpha, A, B, beta, C, opt);
        return zgemm_tp;
    }
...
{
    parsec_zgemm_NN_mtest_taskpool_t *zgemm_tp = (parsec_zgemm_NN_mtest_taskpool_t *)tp;
...
/* no GPU for now */
        ...
        parsec_zgemm_NN_mtest_gpu_taskpool_t *zgemm_gpu_tp = (parsec_zgemm_NN_gpu_taskpool_t *)tp;
...
dplasma_zgemm( parsec_context_t *parsec,
               ...
               dplasma_complex64_t beta,        parsec_tiled_matrix_t *C)
{
    parsec_taskpool_t *parsec_zgemm_mtest = NULL;
    int M, N, K;
    ...
    parsec_zgemm_mtest = dplasma_zgemm_New(transA, transB,
                                    alpha, A, B,
                                    beta, C);

    if ( parsec_zgemm_mtest != NULL )
    {
        parsec_context_add_taskpool( parsec, (parsec_taskpool_t*)parsec_zgemm_mtest);
        dplasma_wait_until_completion(parsec);
        dplasma_zgemm_Destruct( parsec_zgemm_mtest );
        return 0;
    }
    return -101;
}
